...
class face_emotion():
    ...
    def send_result_to_backend(self, mood,start_time,end_time):
        dict = {"mood": mood, "startTime":start_time,"endTime":end_time}
        print(dict)
        json_map = json.dumps(dict)
        self.queue.put(json_map)
            ...
            if (len(faces) != 0):
                ...
                for i in range(len(faces)):
                    ...
                    for k, d in enumerate(faces):
                        ...
                        # print(result)
                        if self.pre_mood != result:
                            self.pre_mood = result
                            cur_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                            self.end_time = cur_time
                            self.send_result_to_backend(self.pre_mood,self.start_time,self.end_time)
...
def start(queue):
    global my_face
    if my_face ==None:
        my_face = face_emotion(queue=queue)
